/*
 * C glue layer for COBOL benchmark server.
 * mongoose HTTP/WS + libpq, calls GnuCOBOL routines.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* COBOL functions (compiled as shared lib) */
extern int cobol_validate_id(int *id, int *result);
extern int cobol_process_name(char *name, int *name_len, char *out, int *out_len);
...
static PGconn *pg_acquire(void) {
    ...
    for (int i = 0; i < POOL_SIZE; i++)
        if (!pg_used[i]) { pg_used[i]=1; pthread_mutex_unlock(&pg_mutex); return pg_pool[i]; }
...
static int pg_pool_init(void) {
    const char *h = getenv("DB_HOST"); if (!h) h = "postgres";
    char ci[256]; snprintf(ci, sizeof(ci), "host=%s port=5432 dbname=bench user=bench password=bench", h);
    for (int a = 0; a < 30; a++) {
        PGconn *t = PQconnectdb(ci);
        if (PQstatus(t)==CONNECTION_OK) { PQfinish(t);
            for (int i=0;i<POOL_SIZE;i++) { pg_pool[i]=PQconnectdb(ci); pg_used[i]=0; }
            return 0;
        }
        PQfinish(t); fprintf(stderr, "DB retry %d/30\n",a+1); sleep(1);
    }
    return -1;
}
...
static int json_get_str(const char *j, size_t jl, const char *key, char *o, size_t os) {
    ...
    const char *p=memmem(j,jl,n,strlen(n)); if(!p) return -1;
    p+=strlen(n); while(*p==' '||*p==':'||*p=='\t')p++;
    if(*p!='"') return -1; p++;
    const char *e=strchr(p,'"'); if(!e) return -1;
    size_t l=(size_t)(e-p); if(l>=os) l=os-1;
    memcpy(o,p,l); o[l]='\0'; return 0;
}
...
static void ev_handler(struct mg_connection *c, int ev, void *ev_data) {
    if (ev == MG_EV_HTTP_MSG) {
        ...
        if (mg_match(hm->uri, mg_str("/items")) && mg_strcmp(hm->method, mg_str("POST"))==0) {
            ...
            /* Call COBOL logic */
            char cobol_out[256]={0}; int nl=(int)strlen(name), ol=0;
            cobol_process_name(name, &nl, cobol_out, &ol);
            ...
            } else mg_http_reply(c,500,"","{\"error\":\"insert failed\"}");
            PQclear(r); pg_release(pg);
            return;
        }

        if (mg_match(hm->uri, mg_str("/items/*"),NULL) && mg_strcmp(hm->method,mg_str("GET"))==0) {
            char id_s[32]={0}; size_t il=hm->uri.len-7;
            if(il>=sizeof(id_s)) il=sizeof(id_s)-1;
            memcpy(id_s, hm->uri.buf+7, il);
            int id=atoi(id_s), valid=0;
            cobol_validate_id(&id, &valid);
            if (!valid) { mg_http_reply(c,400,"","{\"error\":\"invalid id\"}"); return; }
    ...
    } else if (ev==MG_EV_CLOSE && c->is_websocket) hub_remove(c);
}

int main(void) {
    cob_init(0, NULL);  /* Initialize GnuCOBOL runtime */
    if (pg_pool_init()<0) { fprintf(stderr,"DB init failed\n"); return 1; }
    printf("COBOL+C/Mongoose server on :8080\n");
